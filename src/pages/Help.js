            ...
            <div className="col-lg-8 col-md-8 col-12 ">
              ...
              <ToastContainer />
                ...
                <div class="col-lg-6 col-md-6 col-sm-12 col-12 mt-3">
                  <input type="text" required class="form-control input-field  p-3"
                    value={firstName} onChange={(e) => setFirstName(e.target.value)} placeholder="First name" aria-label="First name" />
                </div>
                <div class="col-lg-6 col-md-6 col-sm-12 col-12  mt-3">
                  <input type="text" required className="form-control input-field  p-3"
                    value={lastName} onChange={(e) => setLastName(e.target.value)} placeholder="Last name" aria-label="Last name" />
                </div>
                <div class=" col-12 mt-4">
                  <input type="email" required className="form-control input-field p-3"
                    value={email} onChange={(e) => setEmail(e.target.value.toLowerCase())} placeholder="Email Address" aria-label="Email Address" />
                </div>
                <div class=" col-12 mt-4">
                  <input type="text" required className="form-control input-field p-3"
                    value={subject} onChange={(e) => setSubject(e.target.value)} placeholder="Subject" aria-label="Subject" />
                </div>
                <div class=" col-12 mt-4">
                  <textarea placeholder='Please describe your issue' className='form-control input-field p-3' rows={4}>

                  </textarea>
                </div>
                <div class="col-12 mt-3 ">
                  <div class="form-check colororing">
                    <input class="form-check-input " checked={terms === 1}
                      onChange={(e) => setTerms(e.target.checked ? 1 : 0)} type="checkbox" id="gridCheck" />
                    <label class="form-check-label colorgray" for="gridCheck">
                      I am agreed with Terms of Service, Privacy Policy Provided by GrapeTask.
                    </label>
                  </div>
                  <button type='submit' loading={true} disabled={isLoadingRegister} className='btn-fill mt-4'>
                    {isLoadingRegister ? <Spinner size="sm" color="light" /> : 'Submit'}
                  </button>
